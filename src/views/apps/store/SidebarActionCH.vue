  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
       <div class="p-6">
          ...
          <v-select class="select-store m15" placeholder="Tỉnh / Thành phố" :options="LIST_PROVINCE" v-model="selected_province" label="NAME_PROVINCE" value="ID_PROVINCE">
              
          </v-select>
          
          <v-select class="select-store m15" placeholder="Quận / Huyện" :options="LIST_DISTRICT" v-model="selected_district" label="NAME_DISTRICT" value="ID_DISTRICT"></v-select>
...
export default {
  ...
  watch: {
    selected_province(province)
    {
      if(province != null)
      {
        this.$store.dispatch('fetchDistrict',province.ID_PROVINCE).then((response) => {
          if(Object.entries(this.data).length > 0)
          {
            this.selected_district = response.filter((value,index,array) => {
              return array[index].ID_DISTRICT == this.data.ID_DISTRICT
            })[0]
          }
        })
    ...
    isSidebarActiveLocal(val) {
      ...
      if(Object.entries(this.data).length === 0) {
        ...
        this.selected_province = this.LIST_PROVINCE.filter((value,index,arr) => {
          return arr[index].ID_PROVINCE == this.data.ID_PROVINCE
        })[0]
