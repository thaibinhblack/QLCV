// en, de, fr, pt
export default {
	en: {
		stores: 'CỬA HÀNG HỆ THỐNG',
		store: 'CỬA HÀNG',
		listStore: 'DANH SÁCH CỬA HÀNG',
		product: 'SẢN PHẨM',
		listProduct: 'DANH SÁCH SẢN PHẨM',
		addProduct: 'THÊM MỚI SẢN PHẨM',
		typeProduct: 'THỂ LOẠI SẢN PHẨM',
		log: 'LỊCH SỬ',
		logProduct: 'LỊCH SỬ SẢN PHẨM',
		bill: 'HÓA ĐƠN',
		listBill: 'DANH SÁCH HÓA ĐƠN',
		addBill: 'THÊM MỚI HÓA ĐƠN',
		addBillMore: 'THÊM MỚI HÓA ĐƠN KHÁC',
		logBill: 'LỊCH SỬ HÓA ĐƠN',
		customer: 'KHÁCH HÀNG',
		customers: 'DANH SÁCH KHÁCH HÀNG',
		listCustomer: 'DANH SÁCH KHÁCH HÀNG',
		typeCustomer: 'PHÂN LOẠI KHÁCH HÀNG',
		customerSystem: 'KHÁCH HÀNG CỦA HỆ THỐNG',
		userSystem: 'NHÂN VIÊN HỆ THỐNG',
		user: 'NHÂN VIÊN',
		
	},
	vi: {
		stores: 'CỬA HÀNG HỆ THỐNG'
	}
}
