          ...
          <ul class="list-point">
            <li>
              ...
              <ul class="list-child" :class="{active: bill_more}">
                <li><vs-input class="w-full" min="0" :max="selected_customer.DIEM_TICH_LUY" type="number" label="Đổi điểm thưởng"></vs-input></li>
                <li><vs-input class="w-full" label="Giảm giá (%)"></vs-input></li>
              </ul>
            </li>
          </ul>

          <ul class="total">
              <li><small><strong>*Ghi chú</strong> {{SETTING_POINT.SO_TIEN == 0 ? 'Chưa có cài đặt điểm tích lũy!' : (SETTING_POINT.SO_TIEN).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} VNĐ =  {{SETTING_POINT.SO_TIEN == 0 ? '' : SETTING_POINT.SO_DIEM}}</small></li>
              <li>Tích lũy điểm </li>
            
              <li class="right">{{((SETTING_POINT.SO_TIEN == 0 ? 0 :TOTAL_BILL/SETTING_POINT.SO_TIEN)*SETTING_POINT.SO_DIEM).toFixed(0)}}</li>
              <li>Thành tiền</li>
              <li class="right"> {{TOTAL_BILL.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vnđ</li>
          </ul>
