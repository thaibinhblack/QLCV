    ...
    <div class="section-bill bg-white p-6">
      ...
      <div class="selected_customer" v-if="selected_customer != null">
          ...
          <ul class="total">
              ...
              <li>Thành tiền</li>
              <li class="right"> 
                <p class="money" :class="{active: more.GIAM_GIA > 0 || more.DOI_DIEM > 0}">{{TOTAL_BILL.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vnđ</p>
                <p class="money" v-if="more.GIAM_GIA > 0">{{(TOTAL_BILL - ((TOTAL_BILL/100)*more.GIAM_GIA).toFixed(2)).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vnđ</p>  
              </li>
          </ul>
