...
<vs-row vs-w="12">
  ...
  <vs-col vs-justify="center" vs-align="center" vs-lg="4" vs-sm="12" vs-xs="12" v-if="selected_customer != null">
    ...
    <ul class="list-bill" v-for="(bill,index) in LIST_BILL" :key="index">
      <li>{{bill.TEN_SAN_PHAM}}</li>
      <li>{{bill.GIA_SAN_PHAM}}</li>
      <li>{{bill.SO_LUONG}}</li>
      <li>{{bill.SO_LUONG * bill.GIA_SAN_PHAM}}</li>
    </ul>
  </vs-col>
</vs-row>
</template>
...
.list-product,.list-bill {width:  100%;}
.list-product>li,.list-bill>LI{display: inline-block; width: 25%; padding: 10px 10px 0 0;}
</style>