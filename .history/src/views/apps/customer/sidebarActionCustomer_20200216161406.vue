  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
        <div class="p-6">
          ...
          <vs-input label="Tên sản phẩm" v-model="cusomter.TEN_KH" class="mt-5 w-full" name="TEN_KH" placeholder="Nhập tên sản phẩm" v-validate="'required'" />
          <span class="text-danger text-sm" v-show="errors.has('TEN_KH')">{{ errors.has('TEN_KH') == true ? 'Chưa nhập tên sản phẩm' : '' }}</span>
          <vs-row>
            <div class="mt-6">
               <vs-input type="number" min="0" label="Giá sản phẩm" v-model="cusomter.NGAY_SINH_KH" class="w-full" name="NGAY_SINH_KH" placeholder="Nhập giá sản phẩm" v-validate="'required'" />
              <span class="text-danger text-sm" v-show="errors.has('NGAY_SINH_KH')">{{ errors.has('NGAY_SINH_KH') == true ? 'Chưa nhập giá sản phẩm' : '' }}</span>
            </div>
            <div class="mt-6" style="margin-left:15px;">
              <vs-input type="number" min="0" label="Số lượng sản phẩm" v-model="cusomter.SDT_KH" class="w-full" name="SDT_KH" placeholder="Nhập số lượng sản phẩm" v-validate="'required'" />
              <span class="text-danger text-sm" v-show="errors.has('SDT_KH')">{{ errors.has('SDT_KH') == true ? 'Chưa nhập số lượng sản phẩm' : '' }}</span>
            </div>
            
          </vs-row>
          <vue-editor v-model="cusomter.NOI_DUNG_SAN_PHAM"  style="margin-top: 15px;"/>
          <vs-textarea label="Mô tả sản phẩm" v-model="cusomter.GT_KH" class="mt-5 w-full" name="GT_KH" placeholder="Nhập mô tả sản phẩm"  />
