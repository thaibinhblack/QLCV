...
export default {
  ...
  methods: {
    ...
    FilterProvince()
    {
        this.$store.dispatch('fetchDistrict',this.customer.DC_TP_KH).the((response) => {
          if(Object.entries(this.data).length > 0 )
          {
            
            this.selected_district = response.filter((value,index,array) => {
              return array[index].ID_PROVINCE == this.data.DC_QH_KH
            })[0]
            console.log('filter địa chỉ',this.selected_district)
          }
        })
    },
    SearchCustomerSDT()
