<template>
  <vs-sidebar @keyup.esc="isSidebarActiveLocal = false" click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
        <v-select class="select-store w-full p-6" placeholder="Chọn cửa hàng" v-model="selected_store" :options="LIST_STORE" value="ID_CUA_HANG"  label="TEN_CUA_HANG"  ></v-select><br />
        <vs-input class="w-full p-6" type="text" label="Loại khách hàng" placeholder="Nhập loại khách hàng"></vs-input>
         <vs-input class="w-full p-6" type="text" label="Loại khách hàng" placeholder="Nhập loại khách hàng"></vs-input>
    </VuePerfectScrollbar>
