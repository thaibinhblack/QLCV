<template>
    <div>
        <sidebar-phan-loai :data="sidebarData" />
        <vs-table ref="table" :data="LIST_TYPE_LOCAL" >
          <div style="margin-bottom: 15px;" slot="header" class="header-table-search flex flex-wrap-reverse  flex-grow ">
                ...
                <vs-button style="float: right;margin-left: 15px;" type="border" icon-pack="feather" icon="icon-plus" @click="$store.state.customer.SETTING_PHAN_LOAI = true">Thêm mới</vs-button>
...
export default {
    ...
    methods:
    {
      editData(data)
      {
        ...
        this.$store.state.customer.SETTING_PHAN_LOAI = true
      },
      deleteData(data)
      {
