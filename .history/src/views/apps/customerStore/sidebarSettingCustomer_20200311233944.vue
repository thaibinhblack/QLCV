<template>
  <vs-sidebar @keyup.esc="isSidebarActiveLocal = false" click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    <div class="mt-6 flex items-center justify-between px-6">
        <h4>{{Object.entries(data).length > 0 ? 'CẬP NHẬT PHÂN LOẠI KHÁCH HÀNG' : 'PHÂN LOẠI KHÁCH HÀNG'}}</h4>
        <feather-icon icon="XIcon" @click.stop="isSidebarActiveLocal = false" class="cursor-pointer"></feather-icon>
    </div>
    <vs-divider class="mb-0"></vs-divider>
    <VuePerfectScrollbar class="scroll-area--data-list-add-new">
        <div class="p-6">
            <v-select :disabled="Object.entries(data).length > 0 ? true: false  " class="select-store w-full" placeholder="Chọn cửa hàng" v-model="selected_store" :options="LIST_STORE" value="ID_CUA_HANG"  label="TEN_CUA_HANG"  ></v-select><br />
            ...
            <vs-input v-model="type_customer.SO_TIEN_PHAN_LOAI_MIN" class="w-full mt-15" type="number" label="Số tiền lớn nhất" placeholder="Nhập số tiền lớn nhất"></vs-input>
    ...
    <div class="flex flex-wrap items-center p-6" slot="footer">
      <vs-button class="mr-6" @click="submitAction" >{{Object.entries(data).length > 0 ? 'CẬP NHẬT': 'THÊM MỚI'}}</vs-button>
...
export default {
    ...
    watch:
    {
        ...
        selected_store(store)
        {
            ...
            this.type_customer.TEN_CUA_HANG = store.TEN_CUA_HANG
        },
        data(data)
        {
          if(Object.entries(data).length > 0) {
            this.selected_store = this.LIST_STORE.filter((value,index,array) => {
              return array[index].ID_CUA_HANG == data.ID_CUA_HANG
            })[0]
            this.type_customer = data
          }
          else
          {
    ...
    methods:
    {
        submitAction()
        {
            ...
            {
              this.$store.dispatch("updatePhanLoaiKH",this.type_customer).then((response) => {
                 if(response.success == true)
                  {
                  // this.initValue()
                  this.$vs.notify({
