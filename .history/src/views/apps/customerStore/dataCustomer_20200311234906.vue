    ...
    watch:
    {
        ...
        selected_type(type)
        {
          if(type != null)
          {
            ...
            this.LIST_CUSTOMER_LOCAL = this.LIST_CUSTOMER.filter((value,index,array) => {
              if(IndexType < this.LIST_TYPE.length)
              {
                return array[index].ID_CUA_HANG == type.ID_CUA_HANG && array[index].SO_TIEN_DA_CHI >= type.SO_TIEN_PHAN_LOAI_MAX &&  array[index].SO_TIEN_DA_CHI  < this.LIST_TYPE[IndexType]
              }
              else
              {
                return array[index].ID_CUA_HANG == type.ID_CUA_HANG && array[index].SO_TIEN_DA_CHI >= type.SO_TIEN_PHAN_LOAI_MAX
              }
            })
          }else
          {
    ...
    methods:
    {
      rednerPhanLoaiKH(data)
      {
        const TEN_PHAN_LOAI =  this.LIST_TYPE.filter((value,index,array) => {
          return array[index].ID_CUA_HANG == data.ID_CUA_HANG && array[index].SO_TIEN_PHAN_LOAI_MAX >= data.SO_TIEN_DA_CHI && data.SO_TIEN_DA_CHI >array[index].SO_TIEN_PHAN_LOAI_MIN  
        })
        console.log(TEN_PHAN_LOAI)
        if(TEN_PHAN_LOAI.length > 0)
        {
          return TEN_PHAN_LOAI[TEN_PHAN_LOAI.length - 1].TEN_PHAN_LOAI
        }
        else{
