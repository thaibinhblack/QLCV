<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
       <div class="p-6">
          <v-select class="select-store" placeholder="Chọn cửa hàng" v-model="selected_type_store" :options="TYPE_STORE" 
          value="ID_LOAI_CUA_HANG"  label="TEN_LOAI_CUA_HANG"  ></v-select><br />
          ...
          <div class="section-option" v-if="Object.entries(this.data).length == 0">
             <!-- {{store.OPTION_STORE}} -->
              <vs-input v-for="(setting,index) in SETTING_STORE" :key="index" 
            :label="setting.NAME_ATTRIBUTE"  v-model="store.OPTION_STORE[index].VALUE_ATTRIBUTE" class="mt-5 w-full" :placeholder="setting.NOTE_ATTRIBUTE" v-validate="'required'" />
          </div>
          <div class="section-option" v-else>
              <vs-input v-for="(setting,index) in store.OPTION_STORE" :key="index" 
            :label="SETTING_STORE[index].NAME_ATTRIBUTE"  v-model="store.OPTION_STORE[index].VALUE_ATTRIBUTE" class="mt-5 w-full" :placeholder="setting.NOTE_ATTRIBUTE" v-validate="'required'" />
          </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["SETTING_STORE", "TYPE_STORE"]),
    isFormValid() {
