          ...
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td v-for="(option,index) in SETTING_STORE" :key="index">
               {{RENDER_VALUE_OPTION(tr.OPTION_STORE,index)}}
              </vs-td>
...
export default {
  ...
  methods: {
    ...
    RENDER_VALUE_OPTION(data,index)
    {
     
     console.log('test',index,data, data[index])
      return data[index] == 'undefined' ?  data[index].VALUE_ATTRIBUTE == 'undefined' ? '' : data[index].VALUE_ATTRIBUTE : data[index].VALUE_ATTRIBUTE
    },
    editData(data) {
      this.isSidebarActive = true
