    ...
    <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
        ...
        <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
            <div class="p-6">
                ...
                <span>Cửa hàng</span>
                <v-select v-model="store.ID_PROVINCE" :options="stores" >
                <vs-select-item :key="item.value" :value="item.ID_PROVINCE" :text="item.NAME_PROVINCE" v-for="item in gender" />
            ...
            <vs-list>
                ...
                <vs-list-item v-for="(user,index) in LIST_MANAGER" :key="index"  :title="user.HO_TEN_USER" :subtitle="user.USERNAME_USER">
                    <vs-button color="warning" type="border" icon="close" @click="REMOVE_MANAGER(user)"></vs-button>
                </vs-list-item>          
            </vs-list>
        </VuePerfectScrollbar>
