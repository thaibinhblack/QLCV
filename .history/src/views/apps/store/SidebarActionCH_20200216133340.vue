...
export default {
  ...
  methods: {
    ...
    map_option_store()
    {
      ...
      {
        this.store.OPTION_STORE(option => {
            var option_tmp = option
            this.SETTING_STORE((setting) => {
              if(option.NAME_ATTRIBUTE != setting.NAME_ATTRIBUTE)
              {
                option.NAME_ATTRIBUTE = setting.NAME_ATTRIBUTE
                option.VALUE_ATTRIBUTE = ""
              }
            })
            option.push(option_tmp)
        })
      }
    },
    update_option()
