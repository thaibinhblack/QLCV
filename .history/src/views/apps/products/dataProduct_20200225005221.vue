        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-name font-medium truncate">{{ tr.GIA_SAN_PHAM.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')}} vndd</p>
              </vs-td>
               <vs-td>
...
export default {
    ...
    watch:
    {
      ...
      selected_store(store)
      {
        ...
        // console.log(this.products, this.LIST_PRODUCT.filter((value,index,array) => {
        //   // console.log(array[index])
        //   return array[index].ID_CUA_HANG == store.ID_CUA_HANG
        // }))
      }
