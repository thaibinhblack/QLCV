    ...
    <VuePerfectScrollbar class="scroll-area--data-list-add-new" :settings="settings">
        <div class="p-6">
          ...
          <span class="text-danger text-sm" v-show="errors.has('TEN_SAN_PHAM')">{{ errors.has('TEN_SAN_PHAM') == true ? 'Chưa nhập tên sản phẩm' : '' }}</span>
           <vs-input type="number" min="0" label="Giá sản phẩm" v-model="product.GIA_SAN_PHAM" class="mt-6" name="GIA_SAN_PHAM" placeholder="Nhập giá sản phẩm" v-validate="'required'" />
          <span class="text-danger text-sm" v-show="errors.has('GIA_SAN_PHAM')">{{ errors.has('GIA_SAN_PHAM') == true ? 'Chưa nhập giá sản phẩm' : '' }}</span>
           <vs-input type="number" min="0" label="Số lượng sản phẩm" v-model="product.SO_LUONG_HT" class="mt-6" name="SO_LUONG_HT" placeholder="Nhập số lượng sản phẩm" v-validate="'required'" />
