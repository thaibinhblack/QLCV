    ...
    <div id="data-list-list-view" class="data-list-container">
        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr>
              <vs-td></vs-td>
              <vs-td> <vs-input type="text" v-model="SEARCH_TEN_SAN_PHAM" @change="search('TEN_SAN_PHAM')"> </vs-input></vs-td>
            </vs-tr>
...
export default {
    ...
    watch:
    {
      ...
      SEARCH_TEN_SAN_PHAM(TEN_SAN_PHAM)
      {
        if(TEN_SAN_PHAM != '')
        {
          this.products = this.products.filter((value,index,array) => {
            return (array[index].TEN_SAN_PHAM).toLowerCase().indexOf((TEN_SAN_PHAM).toLowerCase()) > -1
          })
        }
        else{
          this.products = this.LIST_PRODUCT
