...
export default {
    ...
    {
        return new Promise((resolve, reject) => {
            axios.get('/api/customer_ch?SDT_KH='+SDT_KH+'&api_token='+axios.defaults.params.token)
            .then((response) => {
    ...
    {
        return new Promise((resolve, reject) => {
            axios.get('/api/customer_ch?TYPE_STORE='+TYPE_STORE+'&api_token='+axios.defaults.params.token)
            .then((response) => {
                commit("SET_CUSTOMER_SEARCH",response.data.result)
                resolve(response)
            })
